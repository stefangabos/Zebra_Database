 ...
 *  @version    3.0.0 (last revision: July 28, 2026)
 *  @copyright  © 2006 - 2026 Stefan Gabos
...
`     *  // let's assume` that "table" has 100 rows but we're only selecting the first 10 of those
    ...
    /**
     *  Everything we need to remember about the unbuffered query currently being iterated over - the SQL, the index of
     *  its entry in the debugging console, and whether the total number of rows and the EXPLAIN are wanted for it.
     *
     *  Keeping them here works because MySQL only ever lets a single unbuffered result set be open on a connection at a
     *  time - a second query while one is still being read fails with "Commands out of sync".
     *
     *  @var array<mixed>
     *  @access private
     */
    private $unbuffered_info = array();
    ...
    private $mysql_functions = array(
        ...
        'ABS', 'ACOS', 'ADDDATE', 'ADDTIME', 'AES_DECRYPT', 'AES_ENCRYPT', 'ANY_VALUE', 'AREA', 'ASBINARY', 'ASCII', 'ASIN',
        'ASTEXT', 'ASWKB', 'ASWKT', 'ASYMMETRIC_DECRYPT', 'ASYMMETRIC_DERIVE', 'ASYMMETRIC_ENCRYPT', 'ASYMMETRIC_SIGN',
        'ASYMMETRIC_VERIFY', 'ATAN', 'ATAN2', 'AVG', 'BENCHMARK', 'BIN', 'BIN_TO_UUID', 'BIT_AND', 'BIT_COUNT', 'BIT_LENGTH',
        'BIT_OR', 'BIT_XOR', 'BUFFER', 'CAST', 'CEIL', 'CEILING', 'CENTROID', 'CHAR', 'CHARACTER_LENGTH', 'CHARSET',
        'CHAR_LENGTH', 'COALESCE', 'COERCIBILITY', 'COLLATION', 'COMPRESS', 'CONCAT', 'CONCAT_WS', 'CONNECTION_ID', 'CONTAINS',
        'CONV', 'CONVERT', 'CONVERT_TZ', 'CONVEXHULL', 'COS', 'COT', 'COUNT', 'CRC32', 'CREATE_ASYMMETRIC_PRIV_KEY',
        'CREATE_ASYMMETRIC_PUB_KEY', 'CREATE_DH_PARAMETERS', 'CREATE_DIGEST', 'CROSSES', 'CUME_DIST', 'CURDATE', 'CURRENT_DATE',
        'CURRENT_ROLE', 'CURRENT_TIME', 'CURRENT_TIMESTAMP', 'CURRENT_USER', 'CURTIME', 'DATABASE', 'DATE', 'DATEDIFF',
        'DATE_ADD', 'DATE_FORMAT', 'DATE_SUB', 'DAY', 'DAYNAME', 'DAYOFMONTH', 'DAYOFWEEK', 'DAYOFYEAR', 'DECODE', 'DEFAULT',
        'DEGREES', 'DENSE_RANK', 'DES_DECRYPT', 'DES_ENCRYPT', 'DIMENSION', 'DISJOINT', 'DISTANCE', 'ELT', 'ENCODE', 'ENCRYPT',
        'ENDPOINT', 'ENVELOPE', 'EQUALS', 'EXP', 'EXPORT_SET', 'EXTERIORRING', 'EXTRACT', 'EXTRACTVALUE', 'FIELD', 'FIND_IN_SET',
        'FIRST_VALUE', 'FLOOR', 'FORMAT', 'FORMAT_BYTES', 'FORMAT_PICO_TIME', 'FOUND_ROWS', 'FROM_BASE64', 'FROM_DAYS',
        'FROM_UNIXTIME', 'GEOMCOLLFROMTEXT', 'GEOMCOLLFROMWKB', 'GEOMETRYCOLLECTION', 'GEOMETRYCOLLECTIONFROMTEXT',
        'GEOMETRYCOLLECTIONFROMWKB', 'GEOMETRYFROMTEXT', 'GEOMETRYFROMWKB', 'GEOMETRYN', 'GEOMETRYTYPE', 'GEOMFROMTEXT',
        'GEOMFROMWKB', 'GET_FORMAT', 'GET_LOCK', 'GLENGTH', 'GREATEST', 'GROUPING', 'GROUP_CONCAT', 'GTID_SUBSET',
        'GTID_SUBTRACT', 'HEX', 'HOUR', 'ICU_VERSION', 'IF', 'IFNULL', 'IN', 'INET6_ATON', 'INET6_NTOA', 'INET_ATON', 'INET_NTOA',
        'INSERT', 'INSTR', 'INTERIORRINGN', 'INTERSECTS', 'INTERVAL', 'ISCLOSED', 'ISEMPTY', 'ISNULL', 'ISSIMPLE', 'IS_FREE_LOCK',
        'IS_IPV4', 'IS_IPV4_COMPAT', 'IS_IPV4_MAPPED', 'IS_IPV6', 'IS_USED_LOCK', 'IS_UUID', 'JSON_APPEND', 'JSON_ARRAY',
        'JSON_ARRAYAGG', 'JSON_ARRAY_APPEND', 'JSON_ARRAY_INSERT', 'JSON_CONTAINS', 'JSON_CONTAINS_PATH', 'JSON_DEPTH',
        'JSON_EXTRACT', 'JSON_INSERT', 'JSON_KEYS', 'JSON_LENGTH', 'JSON_MERGE', 'JSON_MERGE_PATCH', 'JSON_MERGE_PRESERVE',
        'JSON_OBJECT', 'JSON_OBJECTAGG', 'JSON_OVERLAPS', 'JSON_PRETTY', 'JSON_QUOTE', 'JSON_REMOVE', 'JSON_REPLACE',
        'JSON_SCHEMA_VALID', 'JSON_SCHEMA_VALIDATION_REPORT', 'JSON_SEARCH', 'JSON_SET', 'JSON_STORAGE_FREE', 'JSON_STORAGE_SIZE',
        'JSON_TABLE', 'JSON_TYPE', 'JSON_UNQUOTE', 'JSON_VALID', 'JSON_VALUE', 'LAG', 'LAST_DAY', 'LAST_INSERT_ID', 'LAST_VALUE',
        'LCASE', 'LEAD', 'LEAST', 'LEFT', 'LENGTH', 'LINEFROMTEXT', 'LINEFROMWKB', 'LINESTRING', 'LINESTRINGFROMTEXT',
        'LINESTRINGFROMWKB', 'LN', 'LOAD_FILE', 'LOCALTIME', 'LOCALTIMESTAMP', 'LOCATE', 'LOG', 'LOG10', 'LOG2', 'LOWER', 'LPAD',
        'LTRIM', 'MAKEDATE', 'MAKETIME', 'MAKE_SET', 'MASTER_POS_WAIT', 'MAX', 'MBRCONTAINS', 'MBRCOVEREDBY', 'MBRCOVERS',
        'MBRDISJOINT', 'MBREQUAL', 'MBREQUALS', 'MBRINTERSECTS', 'MBROVERLAPS', 'MBRTOUCHES', 'MBRWITHIN', 'MD5', 'MICROSECOND',
        'MID', 'MIN', 'MINUTE', 'MLINEFROMTEXT', 'MLINEFROMWKB', 'MOD', 'MONTH', 'MONTHNAME', 'MPOINTFROMTEXT', 'MPOINTFROMWKB',
        'MPOLYFROMTEXT', 'MPOLYFROMWKB', 'MULTILINESTRING', 'MULTILINESTRINGFROMTEXT', 'MULTILINESTRINGFROMWKB', 'MULTIPOINT',
        'MULTIPOINTFROMTEXT', 'MULTIPOINTFROMWKB', 'MULTIPOLYGON', 'MULTIPOLYGONFROMTEXT', 'MULTIPOLYGONFROMWKB', 'NAME_CONST',
        'NOW', 'NTH_VALUE', 'NTILE', 'NULLIF', 'NUMGEOMETRIES', 'NUMINTERIORRINGS', 'NUMPOINTS', 'OCT', 'OCTET_LENGTH',
        'OLD_PASSWORD', 'ORD', 'OVERLAPS', 'PASSWORD', 'PERCENT_RANK', 'PERIOD_ADD', 'PERIOD_DIFF', 'PI', 'POINT',
        'POINTFROMTEXT', 'POINTFROMWKB', 'POINTN', 'POLYFROMTEXT', 'POLYFROMWKB', 'POLYGON', 'POLYGONFROMTEXT', 'POLYGONFROMWKB',
        'POSITION', 'POW', 'POWER', 'QUARTER', 'QUOTE', 'RADIANS', 'RAND', 'RANDOM_BYTES', 'RANK', 'REGEXP_INSTR', 'REGEXP_LIKE',
        'REGEXP_REPLACE', 'REGEXP_SUBSTR', 'RELEASE_ALL_LOCKS', 'RELEASE_LOCK', 'REPEAT', 'REPLACE', 'REVERSE', 'RIGHT',
        'ROLES_GRAPHML', 'ROUND', 'ROW_COUNT', 'ROW_NUMBER', 'RPAD', 'RTRIM', 'SCHEMA', 'SECOND', 'SEC_TO_TIME', 'SESSION_USER',
        'SHA', 'SHA1', 'SHA2', 'SIGN', 'SIN', 'SLEEP', 'SOUNDEX', 'SOURCE_POS_WAIT', 'SPACE', 'SQRT', 'SRID', 'STARTPOINT',
        'STATEMENT_DIGEST', 'STATEMENT_DIGEST_TEXT', 'STD', 'STDDEV', 'STDDEV_POP', 'STDDEV_SAMP', 'STRCMP', 'STR_TO_DATE',
        'ST_AREA', 'ST_ASBINARY', 'ST_ASGEOJSON', 'ST_ASTEXT', 'ST_ASWKB', 'ST_ASWKT', 'ST_BUFFER', 'ST_BUFFER_STRATEGY',
        'ST_CENTROID', 'ST_CONTAINS', 'ST_CONVEXHULL', 'ST_CROSSES', 'ST_DIFFERENCE', 'ST_DIMENSION', 'ST_DISJOINT',
        'ST_DISTANCE', 'ST_DISTANCE_SPHERE', 'ST_ENDPOINT', 'ST_ENVELOPE', 'ST_EQUALS', 'ST_EXTERIORRING', 'ST_GEOHASH',
        'ST_GEOMCOLLFROMTEXT', 'ST_GEOMCOLLFROMTXT', 'ST_GEOMCOLLFROMWKB', 'ST_GEOMETRYCOLLECTIONFROMTEXT',
        'ST_GEOMETRYCOLLECTIONFROMWKB', 'ST_GEOMETRYFROMTEXT', 'ST_GEOMETRYFROMWKB', 'ST_GEOMETRYN', 'ST_GEOMETRYTYPE',
        'ST_GEOMFROMGEOJSON', 'ST_GEOMFROMTEXT', 'ST_GEOMFROMWKB', 'ST_INTERIORRINGN', 'ST_INTERSECTION', 'ST_INTERSECTS',
        ...
        'ST_MULTILINESTRINGFROMTEXT', 'ST_MULTILINESTRINGFROMWKB', 'ST_MULTIPOINTFROMTEXT', 'ST_MULTIPOINTFROMWKB',
        'ST_MULTIPOLYGONFROMTEXT', 'ST_MULTIPOLYGONFROMWKB', 'ST_NUMGEOMETRIES', 'ST_NUMINTERIORRING', 'ST_NUMINTERIORRINGS',
        'ST_NUMPOINTS', 'ST_OVERLAPS', 'ST_POINTFROMGEOHASH', 'ST_POINTFROMTEXT', 'ST_POINTFROMWKB', 'ST_POINTN',
        'ST_POLYFROMTEXT', 'ST_POLYFROMWKB', 'ST_POLYGONFROMTEXT', 'ST_POLYGONFROMWKB', 'ST_SIMPLIFY', 'ST_SRID', 'ST_STARTPOINT',
        'ST_SYMDIFFERENCE', 'ST_TOUCHES', 'ST_UNION', 'ST_VALIDATE', 'ST_WITHIN', 'ST_X', 'ST_Y', 'SUBDATE', 'SUBSTR',
        'SUBSTRING', 'SUBSTRING_INDEX', 'SUBTIME', 'SUM', 'SYSDATE', 'SYSTEM_USER', 'TAN', 'TIME', 'TIMEDIFF', 'TIMESTAMP',
        'TIMESTAMPADD', 'TIMESTAMPDIFF', 'TIME_FORMAT', 'TIME_TO_SEC', 'TOUCHES', 'TO_BASE64', 'TO_DAYS', 'TO_SECONDS', 'TRIM',
        'TRUNCATE', 'UCASE', 'UNCOMPRESS', 'UNCOMPRESSED_LENGTH', 'UNHEX', 'UNIX_TIMESTAMP', 'UPDATEXML', 'UPPER', 'USER',
        'UTC_DATE', 'UTC_TIME', 'UTC_TIMESTAMP', 'UUID', 'UUID_SHORT', 'UUID_TO_BIN', 'VALIDATE_PASSWORD_STRENGTH', 'VALUES',
        'VARIANCE', 'VAR_POP', 'VAR_SAMP', 'VERSION', 'WAIT_FOR_EXECUTED_GTID_SET', 'WAIT_UNTIL_SQL_THREAD_AFTER_GTIDS', 'WEEK',
    ...
    */
     ...
     *                                  The caching method is specified through the {@link caching_method} property.
     *
     *                                  *Only queries that return a result set are cached (`SELECT`, `SHOW`, `DESCRIBE`
     *                                  and `EXPLAIN`). For queries like `INSERT`, `UPDATE` or `DELETE`, this argument
     *                                  is ignored and the query is executed every time.*
     *
    ...
    public function query($sql, $replacements = '', $cache = false, $calc_rows = false, $highlight = false) {
        ...
        // whatever we remembered about a previous unbuffered query does not apply to this one
        $this->unbuffered_info = array();
        ...
        if ($replacements !== '' && is_array($replacements) && !empty($replacements)) {
            ...
            foreach ($replacements as $key => $replacement) {
                ...
                // (an empty array becomes a subquery returning no rows, which is the only value that is correct for both "IN (?)" - which then matches nothing - and "NOT IN (?)" - which matches everything)
                elseif (is_array($replacement)) $replacements2[$key] = empty($replacement) ? '(SELECT 1 FROM DUAL WHERE FALSE)' : preg_replace(array('/\\\\/', '/\$([0-9]*)/'), array('\\\\\\\\', '\\\$$1'), $this->implode($replacement));
        ...
        if ($this->last_result !== false) {
            ...
            if ($this->_is_result($this->last_result) || $this->last_result === true) {
                ...
                if ($returns_rows) {
                    ...
                    // for unbuffered queries remember the SQL so that once all the rows have been iterated over, we can
                    // get some extra information (see the _manage_unbuffered_query_info method for more info)
                    else $this->unbuffered_info['query'] = $sql;
            ...
            if ($calc_rows && $this->unbuffered && $this->_is_result($this->last_result))
                ...
                $this->unbuffered_info['calc_rows'] = true;
            ...
            if ($this->_is_debugging_enabled()) {
                ...
                if ($returns_rows) {
                    ...
                    if (
                        ...
                        $this->unbuffered_info['explain'] = true;

                        // the SQL to explain
                        $this->unbuffered_info['query'] = $sql;
                ...
                if ($this->unbuffered && $returns_rows && $this->_is_result($this->last_result))
                    ...
                    $this->unbuffered_info['log_index'] = count($this->debug_info['successful-queries']) - 1;
    ...
    private function _manage_unbuffered_query_info(&$resource, &$result) {
        ...
        // the index of this query's entry in the debugging console
        // (it is only ever set for an unbuffered query of ours, so anything else has nothing to update)
        $log_index = isset($this->unbuffered_info['log_index']) ? $this->unbuffered_info['log_index'] : null;
        ...
        if (!$result) {
            ...
            if ($this->_is_debugging_enabled() && $log_index !== null) {

                // update the number of returned rows in the debugging console
                $this->debug_info['successful-queries'][$log_index]['returned_rows'] = $resource->num_rows;

                // if we need to get the total number of rows in the table
                if (!empty($this->unbuffered_info['calc_rows'])) {
                ...
                if (!empty($this->unbuffered_info['explain'])) {
        ...
        } elseif ($this->_is_debugging_enabled() && $log_index !== null && count($this->debug_info['successful-queries'][$log_index]['records']) < $this->debug_show_records)

            // add row data to the debugging console
            $this->debug_info['successful-queries'][$log_index]['records'][] = $result;
