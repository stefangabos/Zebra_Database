...
# value, so they can be killed again afterwards:
#
#   redis-server --port 6379 --save '' --appendonly no &
#   memcached -p 11211 -m 64 &
#
# That is 8 tests covering the memcache and redis caching backends, which are otherwise not exercised at
# all. The socket test works out the socket path from the server itself, so it needs no setup.
#
